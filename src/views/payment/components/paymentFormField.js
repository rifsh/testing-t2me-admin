  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const transformedData = {
        ...
        // Transform QR payment details
        payment_qr_details:
          values.qrPayments?.map((qr) => ({
            provider_name: qr.paymentProvider,
            qr_code: qr.qrCode?.[0]?.originFileObj, // Get the actual file
          })) || [],
        ...
        // Transform add-on services
        add_on_services:
          values.services?.map((service) => ({
            name: service.name,
            image: service.image?.[0]?.originFileObj, // Get the actual file
          })) || [],
      };
