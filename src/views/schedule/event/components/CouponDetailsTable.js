    ...
    const couponColumns = [
        {
            ...
            render: (text, record) => (
                <div className="flex items-center">
                    {record.coupons?.thumbnail_image !== "images" && record.coupons?.thumbnail_image ? (
                        <Image
        ...
        },
        {
            ...
            render: (text, record) => (
                <Tag color="green">
                    {text}{record.coupons?.is_percentage ? '%' : ''}
                </Tag>
            )
        },
        {
        ...
        },
        {
            ...
            render: (_, record) => (
                <Text>
                    {record.coupons?.start_date ? Utils.formatDate(record.coupons.start_date) : 'No start date'} - {' '}
                    {record.coupons?.end_date ? Utils.formatDate(record.coupons.end_date) : 'No end date'}
                </Text>
            )
        },
        {
            ...
            render: (_, record) => (
                <Text>
                    {Utils.formatDate(record.valid_from)} - {Utils.formatDate(record.valid_to)}
                </Text>
            )
        },
        {
                                ...
                                expandedRowRender: (record) => (
                                    <div className="p-4 bg-gray-50 rounded">
                                        <Descriptions bordered column={2}>
                                            <Descriptions.Item label="Max Uses">{record.coupons?.max_uses}</Descriptions.Item>
                                            <Descriptions.Item label="Used Count">{record.coupons?.used_count}</Descriptions.Item>
