...
import {
  createTheaterCompany,
  editTheaterCompany,
  fetchTheaterCompanyByid,
  setTheaterCompanyEditData,
} from "store/slices/theaterCompanySlice";
...
const CompanyForm = ({ mode, CompanyEditId }) => {
  ...
  const {
    ...
    submitMessage,
    singleResponse,
    message: theaterCompanyMessages,
    editData,
  } = useSelector((state) => state.theaterCompany);
  const { selectedPlace, selectedVenue, dialogVisible } = useSelector(
  ...
  useEffect(() => {
    if (mode === MODE.EDIT && singleResponse) {
      form.setFieldsValue({
        ...
        thumbnail_image:
          singleResponse.thumbnail_image &&
          singleResponse.thumbnail_image !== "images"
            ? [
                {
                  uid: "-1",
  ...
  return (
    <div style={{ padding: "0 12px" }}>
      ...
      <Form layout="vertical" form={form} name="theaterForm">
        <Card>
          <>
            <Row gutter={24}>
              <Col xs={24} md={12}>
              ...
              <Col xs={24} md={12}>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Please input company valid mail!",
                    },
                  ]}
                >
                  <Input placeholder="Enter company mail" size="large" />
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item
            ...
            <Form.Item
              ...
              rules={[
                { required: true, message: "Please add a profile image" },
              ]}
              valuePropName="value"
      ...
      <SubmitAndConfirmModal
        responseData={response}
        addFunction={
          mode === MODE.ADD ? createTheaterCompany : editTheaterCompany
        }
