...
import { Form, Button, Row, Typography, message, Space, Tabs } from "antd";
...
import {
  createPersonality,
  editPersonality,
  fetchPersonalitiesById,
  setPersonalityEditData,
    ...
    if (response) {
      ...
      form.setFieldsValue({
        ...
        birthDate: response.birthDate
          ? dayjs(response.birthDate, "YYYY-MM-DD")
          : undefined,
        age: response.age,
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (mode === "ADD") {
        ...
        dispatch(
          createPersonality({ data: formattedData, action: ActionType.SUBMIT })
        );
    ...
    <div>
      ...
      >
        <Tabs defaultActiveKey="1">
          <Tabs.TabPane tab={<span>Actor Details</span>} key="1">
            <ActorDetails form={form} />
          </Tabs.TabPane>
        </Tabs>
