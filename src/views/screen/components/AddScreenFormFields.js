...
const AddScreenFormFields = ({ mode, screenId }) => {
  ...
  const { dialogVisible, selectedVenue } = useSelector(
    (state) => state.locations
  );
  const {
    selectedTheaterId,
    singleResponse: singleTheaterResponse,
    selectedTheaterScreenCapacity,
          ...
          screens: [
            {
              ...
              accessibility:
                singleResponse.accessibilty.map((values) => values.id) || [],
  ...
  const handlePlaceSelect = (id) => {
    ...
    form.resetFields(["venue_id", "theatre_id", "screens"]);
        ...
        if (validateVenue.fulfilled.match(resultAction)) {
          ...
          if (response.message === "warning") {
            ...
            const editResult = await dispatch(
              editScreen({ data: updatedScreen, action: ActionType.WARNING })
            );
                    ...
                    >
                      <Title level={4}>
                        ...
                        <Text type="secondary" strong>
                          (Maximum{" "}
                          <Text type="danger">{`${selectedTheaterScreenCapacity} Screens`}</Text>
                          )
                        </Text>
                      </Title>
                      {mode === "ADD" && (
