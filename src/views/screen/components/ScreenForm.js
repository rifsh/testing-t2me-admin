...
const ScreenForm = ({
  ...
  const rules = {
    ...
    thumbnail_image: [
      { required: true, message: "Please choose a screen image" },
    ],
    ticket_structure: [
      { required: true, message: "Please select a ticket structure" },
    ],
    ticket_sets: [
      { required: true, message: "Please select at least one ticket set" },
    ],
    accessibility: [
      {
        required: true,
    ...
    screen_technology_id: [
      {
        required: true,
        message: "Please select at least one Screen technology",
      },
    ],
    audio_id: [
      {
        required: true,
        message: "Please select at least one Audio technology",
      },
    ],
  };
  ...
  useEffect(() => {
    ...
    setDataStatus({
      technologiesEmpty:
        !techLoading &&
        (!screenTechnologies || screenTechnologies.length === 0),
      audioEmpty:
        !techLoading &&
        (!screenAudioTechnologies || screenAudioTechnologies.length === 0),
      featuresEmpty:
        !techLoading && (!screenFeatures || screenFeatures.length === 0),
    });
  }, [
    screenTechnologies,
    screenAudioTechnologies,
  ...
  const showAlert =
    dataStatus.technologiesEmpty ||
    dataStatus.audioEmpty ||
    dataStatus.featuresEmpty;
  const generateAlertMessage = () => {
    ...
    return `No data available for: ${missingData.join(
      ", "
    )}. Please add data before proceeding.`;
  };
  ...
  return (
    <div className="screen-form-container">
      ...
      <Row gutter={16}>
        <Col xs={24} md={12}>
          <Form.Item
            name={["screens", index, "screen_name"]}
            label="Name"
            rules={rules.subject}
          >
        ...
        <Col xs={24} md={12}>
          <Form.Item
            name={["screens", index, "screen_type"]}
            label="Screen Type"
            rules={rules.screen_type}
          >
      ...
      <Card title="Additional Settings" style={{ marginBottom: "20px" }}>
        <Row gutter={16}>
          <Col xs={24} md={12}>
            <Form.Item
              name={["screens", index, "accessibility"]}
          ...
          <Col xs={24} md={12}>
            <Form.Item
              name={["screens", index, "screen_technology_id"]}
              label="Screen Technology"
              rules={rules.screen_technology_id}
            >
        ...
        <Form.Item
          name={["screens", index, "audio_id"]}
          label="Audio System"
          rules={rules.audio_id}
        >
