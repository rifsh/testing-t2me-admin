...
class Utils {
  ...
  // ================================================================
  /**
   * Encrypt a scalar value (id, action, etc.) for use in query strings.
   * Always returns a Base64 string generated by AES-CBC.
   * @param {String|Number} value  Plain value you want to protect
   * @returns {Promise<String>}    Base64( IV ∥ ciphertext )
   */
  static encryptASL = async (value) => {
    if (value === null || value === undefined) return value;
    // Convert everything to string so decrypt side is predictable
    const plain = String(value);
    return encryptAES(plain, ASL_AES_KEY);
  };

  /**
   * Decrypt a value produced by encryptASL().
   * @param {String} encryptedBase64  The Base64 string from the URL
   * @returns {Promise<String>}       The original plain value as string
   */
  static decryptASL = async (encryptedBase64) => {
    if (!encryptedBase64) return encryptedBase64;
    return decryptAES(encryptedBase64, ASL_AES_KEY);
  };

  /**
   * Convenience: take a plain params object and return an object whose
   * values are ASL-encrypted.  Undefined/null values are stripped out
   * the same way `filterParams` does.
   * @example
   *    Utils.encryptParams({ place_id: 17, page: 1 })
   *       → { place_id: "qB9z…==", page: "4n2P…==" }
   */
  static encryptParams = async (params = {}) => {
    const plain = Utils.filterParams(params);
    const encrypted = {};
    for (const [k, v] of Object.entries(plain)) {
      encrypted[k] = await Utils.encryptASL(v);
    }
    return encrypted;
  };
