...
/**
 * Extracts file objects from form data for specified fields
 * @param {Object} formValues - Form values from Ant Design form
 * @param {Array} fileFields - Array of field names to extract (optional)
 * @returns {Object} - Object containing file objects
 */
export const extractFileObjects = (formValues, fileFields = null) => {
  // Default file fields if not specified
  const defaultFields = {
  ...
  Object.entries(fieldsToExtract).forEach(([fieldName, isArray]) => {
    if (isArray) {
      // Handle array of files
      files[fieldName] = Array.isArray(formValues[fieldName])
        ? formValues[fieldName].map((img) => img.originFileObj).filter(Boolean)
        : [];
    } else {
      // Handle single file
      files[fieldName] = formValues[fieldName]?.[0]?.originFileObj || null;
    }
  });
...
export const UPLOAD_FIELD_CONFIGS = {
  ...
  EVENT: [
    ...
    {
      dataField: "event_images",
      uploadUrlField: "event_images_upload_url",
      fileField: "event_images",
      isArray: true,
    },
  ],
  OFFER: [
